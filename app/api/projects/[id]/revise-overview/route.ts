...
import { hydratePipelineInput, reviseCreativeOverviewWithFeedback, type ArkPipelineState } from "@/lib/pipeline";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (feedback.length < 2 || feedback.length > 1000 || !Number.isInteger(body.revision)) {
      return Response.json({ error: "请提供2到1000字的总览修改意见和有效版本号" }, { status: 422 });
    }
    ...
    if (row.status !== "awaiting_review" || !row.pipelineJson) return Response.json({ error: "当前任务不在创意素材总览确认阶段" }, { status: 409 });

    const currentPipelineJson = row.pipelineJson;
    const state = JSON.parse(currentPipelineJson) as ArkPipelineState;
    if (!["awaiting_image_plan", "awaiting_asset_image_review"].includes(state.phase)) return Response.json({ error: "四幕分镜规划已经开始，创意素材总览不能再次修改" }, { status: 409 });
    if (state.revision !== body.revision) return Response.json({ error: "创意素材总览版本已更新，请刷新后重试" }, { status: 409 });

    const input = hydratePipelineInput(JSON.parse(row.inputJson), row.id, row.title);
    const snapshot = await reviseCreativeOverviewWithFeedback({ input, state, feedback });
    ...
    if (!updated) return Response.json({ error: "创意素材总览已被其他操作更新，请刷新后重试" }, { status: 409 });
    return Response.json({ project: presentProject(updated) });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "创意素材总览重新生成失败" }, { status: 500 });
  }
}
