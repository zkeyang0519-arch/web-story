...
import { hydratePipelineInput, reviseAssetCardWithFeedback, type ArkPipelineState } from "@/lib/pipeline";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (row.status !== "awaiting_review" || !row.pipelineJson) return Response.json({ error: "当前任务不在资产创意卡确认阶段" }, { status: 409 });

    const currentPipelineJson = row.pipelineJson;
    const state = JSON.parse(currentPipelineJson) as ArkPipelineState;
    if (state.phase !== "awaiting_image_plan") return Response.json({ error: "资产创意卡已锁定，不能再次修改" }, { status: 409 });
    if (state.revision !== body.revision) return Response.json({ error: "资产创意卡版本已更新，请刷新后重试" }, { status: 409 });

    const input = hydratePipelineInput(JSON.parse(row.inputJson), row.id, row.title);
    const snapshot = await reviseAssetCardWithFeedback({ input, state, assetId, feedback, draftImagePlan: body.draftImagePlan });
    ...
    if (!updated) return Response.json({ error: "资产创意卡已被其他操作更新，请刷新后重试" }, { status: 409 });
    return Response.json({ project: presentProject(updated) });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "资产描述重新生成失败" }, { status: 500 });
  }
}
