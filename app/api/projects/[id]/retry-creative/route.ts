...
import { retryCreativeSynthesis, type ArkPipelineState } from "@/lib/pipeline";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "创意融合重试失败" }, { status: 500 });
  }
}
