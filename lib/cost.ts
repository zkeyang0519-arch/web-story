...
export function calculateCostQuote(referenceCount: number, duration: number): CostQuote {
  ...
  const generation = { min: money(4.5 * generationFactor), max: money(8 * generationFactor) };
  ...
  return {
    ...
    generation,
    storage,
    totalMin: money(analysis.min + generation.min + storage.min),
    totalMax: money(analysis.max + generation.max + storage.max),
  };
}
